            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  @change="changeSkuNum"
                />
                <a href="javascript:" class="plus" @click="nums++">+</a>
                <a
                  href="javascript:"
                  class="mins"
                  @click="nums == 1 ? 1 : nums--"
...
export default {
  ...
  methods: {
    // 选择商品属性,选中的属性高亮,排他法：先全都置为0，当前选择的这个置为1
    changeActive(attr, spuSaleAttrValueList) {
      spuSaleAttrValueList.forEach((item) => {
        item.isChecked = 0;
      });
      attr.isChecked = 1;
    },
    // input框里nums数量变化的监听事件
    changeSkuNum(e) {
      // 输进去的值是个字符，灰色的，进行强制类型转换
      let nums = e.target.value * 1;
      if (nums <= 0 || isNaN(nums)) nums = 1;
      nums = parseInt(nums);
      this.nums = nums;
    },
    async goCartSuccess() {
